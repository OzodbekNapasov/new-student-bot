...
app.get('/api/v1/health', (req, res) => {
  res.json({ status: 'ok', timestamp: new Date().toISOString(), platform: 'Student Management Platform' });
});
...
app.patch('/api/v1/admin/groups/:groupId/leader', authenticateJWT, authorizeRoles('SUPER_ADMIN'), assignGroupLeader);
...
app.get('/api/v1/leader/groups', authenticateJWT, authorizeRoles('GROUP_LEADER', 'SUPER_ADMIN'), getLeaderGroups);
app.get('/api/v1/leader/groups/:groupId/students', authenticateJWT, authorizeRoles('GROUP_LEADER', 'SUPER_ADMIN'), getGroupStudents);
app.post('/api/v1/leader/attendance', authenticateJWT, authorizeRoles('GROUP_LEADER', 'SUPER_ADMIN'), async (req, res) => {
  await markGroupAttendance(req, res);
  io.emit('attendance_updated', { groupId: req.body.groupId, date: req.body.date });
});
app.post('/api/v1/leader/tasks', authenticateJWT, authorizeRoles('GROUP_LEADER', 'SUPER_ADMIN'), createGroupTask);
app.patch('/api/v1/leader/submissions/:submissionId', authenticateJWT, authorizeRoles('GROUP_LEADER', 'SUPER_ADMIN'), gradeSubmission);
...
server.listen(PORT, () => {
  ...
  if (process.env.TELEGRAM_BOT_TOKEN && process.env.TELEGRAM_BOT_TOKEN !== '123456789:ABCdefGhIJKlmNoPQRsTUVwxyZ') {
    bot.start({
      onStart: (botInfo) => {
