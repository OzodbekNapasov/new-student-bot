    ...
    <div style={{ minHeight: '100vh', paddingBottom: 24 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, position: 'relative' }}>
          <div className="avatar avatar-lg" style={{ background: 'linear-gradient(135deg, #11998e, #38ef7d)', fontSize: 24 }}>🎓</div>
          <div>
            <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.6)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1 }}>Talaba</p>
            <h1 style={{ fontSize: 22, fontWeight: 800 }}>{user.first_name} {user.last_name}</h1>
            {myStudentRecord?.group && (
              <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.6)' }}>📚 {(myStudentRecord.group as any)?.name}</p>
            )}
          </div>
        </div>
      </div>
          ...
          <div className="animate-in">
            ...
            <div className="card">
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                ...
                <span style={{ fontSize: 14, fontWeight: 800, color: percentage >= 80 ? '#34d399' : '#f87171' }}>{percentage}%</span>
              </div>
            ...
            {!myStudentRecord && (
              <div className="card" style={{ marginTop: 16, textAlign: 'center', padding: '32px 20px' }}>
                ...
                <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>Guruh rahbari yoki admin bilan bog'laning.</p>
              </div>
            )}
          </div>
        )}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {[...attendance].sort((a, b) => b.date.localeCompare(a.date)).map(a => (
                  <div key={a.id} className="card" style={{
                    ...
                    <div style={{ flex: 1 }}>
                      <p style={{ fontWeight: 600, fontSize: 14 }}>{new Date(a.date).toLocaleDateString('uz-UZ', { weekday: 'short', month: 'long', day: 'numeric' })}</p>
                      <p style={{ fontSize: 12, color: statusColor[a.status] }}>{statusLabel[a.status]}</p>
                    </div>
